...
const aliasesMX16 = ['mx', 'méxico', 'mexico', 'méx', 'mex']
const aliasesCO16 = ['co', 'colombia', 'col']

let handler16vs16 = async (m, { conn, args }) => {
  if (args.length === 0) {
  ...
  let zonaInput = null
  if (aliasesMX16.includes(lastArg)) {
    zonaInput = 'mx'
    args.pop()
  } else if (aliasesCO16.includes(lastArg)) {
    zonaInput = 'co'
    args.pop()
  } else {
  ...
  const template = generarVersus16vs16([], [], mexText, colText)
  const sent = await conn.sendMessage(m.chat, { text: template, mentions: [] })

  versusData16[sent.key.id] = {
    chat: m.chat,
    escuadra: [],
    suplentes: [],
    mexText,
...
handler16vs16.tags = ['Games']
handler16vs16.command = /^\.?(16vs16|vs16)$/i
handler16vs16.group = true
export default handler16vs16

function generarVersus16vs16(escuadra, suplentes, mexText = '  ', colText = '  ') {
  function formatEscuadra(arr) {
    let out = ''
    for (let i = 0; i < 16; i++) { // máximo 16 jugadores
      let icon = i === 0 ? '👑' : '🥷🏻'
...
conn.ev.on('messages.upsert', async ({ messages }) => {
  for (let msg of messages) {
    ...
    if (emoji === '❌' && isAdmin) {
      ...
      let nuevoTexto = generarVersus16vs16(data.escuadra, data.suplentes, data.mexText, data.colText)
      ...
      delete versusData16[msgID]
      versusData16[sent.key.id] = data
      continue
    }
    ...
    if (emoji === '❤️') {
      if (data.escuadra.length < 16) data.escuadra.push(user)
    } else if (emoji === '👍') {
      if (data.suplentes.length < 8) data.suplentes.push(user)
    } else continue
