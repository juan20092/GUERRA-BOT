...
let handler12vs12 = async (m, { conn, args }) => {
  ...
  let zonaInput = null
  if (aliasesMX12.includes(lastArg)) {
    zonaInput = 'mx'
    args.pop()
  } else if (aliasesCO12.includes(lastArg)) {
    zonaInput = 'co'
    args.pop()
  } else {
...
function generarVersus12vs12(escuadra, suplentes, mexText = '  ', colText = '  ') {
  function formatEscuadra(arr) {
    let out = ''
    for (let i = 0; i < 12; i++) { // máximo 12 jugadores
      let icon = i === 0 ? '👑' : '🥷🏻'
...
conn.ev.on('messages.upsert', async ({ messages }) => {
  for (let msg of messages) {
    ...
    if (emoji === '❤️') {
      ...
      if (data.suplentes.length < 6) data.suplentes.push(user)
    } else continue
